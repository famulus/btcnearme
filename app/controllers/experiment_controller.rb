class ExperimentController < ApplicationController
	...
	def create_post
		...
		if @post.valid?
			redirect_to({:controller => :experiment,:action => :index}, :flash => {:notice => "Post success!"})
		else
			redirect_to({:controller => :experiment,:action => :index}, :flash => {:error => @post.errors.full_messages.join(", ")})
		end
	end
	...
	def delete_post
		...
		redirect_to({:controller => :experiment,:action => :index}, :flash => {:notice => "Email deleted sucessfully"})
	end 
	...
	def remove_email_post
		...
		redirect_to({:controller => :experiment,:action => :index}, :flash => {:notice => message})
